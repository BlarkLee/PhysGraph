...
class DexHandManipBiHEnv(VecTask):
    ...
    def _create_envs(self):
        ...
        self.num_dexhand_rh_bodies = self.gym.get_asset_rigid_body_count(dexhand_rh_asset) #28
        self.num_dexhand_rh_dofs = self.gym.get_asset_dof_count(dexhand_rh_asset) #22
        ...
        num_dexhand_rh_shapes = self.gym.get_asset_rigid_shape_count(dexhand_rh_asset) #16
    ...
    def init_data(self):
        ...
        #print("self.dexhand_rh_handles", self.dexhand_rh_handles)
        ...
        self._root_state = gymtorch.wrap_tensor(_actor_root_state_tensor).view(self.num_envs, -1, 13) #(4096,5,13)
        self._dof_state = gymtorch.wrap_tensor(_dof_state_tensor).view(self.num_envs, -1, 2) #(4096, 44, 2)
        self._rigid_body_state = gymtorch.wrap_tensor(_rigid_body_state_tensor).view(self.num_envs, -1, 13) #(4096,59,13)
        ...
        self.net_cf = gymtorch.wrap_tensor(_net_cf).view(self.num_envs, -1, 3) #(4096,59,3)
        self.dof_force = gymtorch.wrap_tensor(_dof_force).view(self.num_envs, -1) #(4096,44)
        ...
        self._manip_obj_rh_cf = self.net_cf[:, self._manip_obj_rh_rigid_body_handle, :] #3
        self._manip_obj_lh_cf = self.net_cf[:, self._manip_obj_lh_rigid_body_handle, :] #4

        self.dexhand_rh_root_state = self._root_state[:, dexhand_rh_handle, :] #1 overlap with self._rh_base_state
        self.dexhand_lh_root_state = self._root_state[:, dexhand_lh_handle, :] #2
        self.rh_dof_force = self.dof_force[:, : self.num_dexhand_rh_dofs]
        self.lh_dof_force = self.dof_force[:, self.num_dexhand_rh_dofs:]
    ...
    def compute_reward(self, actions):
        lh_rew_buf, lh_reset_buf, lh_success_buf, lh_failure_buf, lh_reward_dict, lh_error_buf, lh_diff_obj_pos, lh_diff_obj_rot, lh_diff_joints, lh_diff_ft = (
            self.compute_reward_side(actions, side="lh")
        )
        rh_rew_buf, rh_reset_buf, rh_success_buf, rh_failure_buf, rh_reward_dict, rh_error_buf, rh_diff_obj_pos, rh_diff_obj_rot, rh_diff_joints, rh_diff_ft = (
            self.compute_reward_side(actions, side="rh")
        )
        self.rew_buf = rh_rew_buf + lh_rew_buf
        self.diff_obj_pos = (rh_diff_obj_pos + lh_diff_obj_pos)/2
        self.diff_obj_rot = (rh_diff_obj_rot + lh_diff_obj_rot)/2
        self.diff_joints = (rh_diff_joints + lh_diff_joints)/2
        self.diff_ft = (rh_diff_ft + lh_diff_ft)/2
    ...
    def compute_reward_side(self, actions, side="rh"):
        ...
        rew_buf, reset_buf, success_buf, failure_buf, reward_dict, error_buf, diff_obj_pos, diff_obj_rot, diff_joints, diff_ft = compute_imitation_reward(
            self.reset_buf,
            self.progress_buf,
    ...
    def compute_observations_side(self, side="rh"):
        ...
        # side_states ['q', 'cos_q', 'sin_q', 'dq', 'base_state', 'joints_state', 'manip_obj_pos', 
        # 'manip_obj_quat', 'manip_obj_vel', 'manip_obj_ang_vel']
        side_states = getattr(self, f"{side}_states")

        # ['seq_len', 'data_path', 'obj_id', 'obj_mesh_path', 'obj_verts', 
        # 'obj_trajectory', 'scene_objs', 'wrist_pos', 'wrist_rot', 'mano_joints', 
        # 'obj_urdf_path', 'tips_distance', 'obj_velocity', 'obj_angular_velocity', 
        # 'wrist_velocity', 'wrist_angular_velocity', 'mano_joints_velocity', 'opt_wrist_pos', 
        # 'opt_wrist_rot', 'opt_dof_pos', 'opt_wrist_velocity', 'opt_wrist_angular_velocity', 'opt_dof_velocity']
        side_demo_data = getattr(self, f"demo_data_{side}")

        obs_dict = {}

        obs_values = []
        for ob in self._obs_keys: # ['q'(22), 'cos_q'(22), 'sin_q'(22), 'base_state'(13)]
            if ob == "base_state":
                obs_values.append(
                    torch.cat([torch.zeros_like(side_states[ob][:, :3]), side_states[ob][:, 3:]], dim=-1)
                )  # ! ignore base position (the world coordinate translation)
            else:
                obs_values.append(side_states[ob])

        obs_dict["proprioception"] = torch.cat(obs_values, dim=-1) #(4096,79)
        ...
        if len(self._privileged_obs_keys) > 0:
            ...
            for ob in self._privileged_obs_keys:
                if ob == "manip_obj_pos":
                    ...
                    )  # add force magnitude, (4096,5,4), 5 is five fingers, 4 is x y z force magnitude
        ...
        cur_idx = self.progress_buf-1 #5 #1
        ...
        nE, nT = side_demo_data["wrist_pos"].shape[:2] #nE=B, nT = max timesteps in the demo buffers.
    ...
    def step(self, actions):
        #stop
        obs, rew, done, info, diff_obj_pos, diff_obj_rot, diff_joints, diff_ft = super().step(actions)
    ...
    def pre_physics_step(self, actions):
        ...
        # ? <<< for visualization
        root_control_dim = 9 if self.use_pid_control else 6 #false so res is half
        res_split_idx = (
            actions.shape[1] // 2
...
def compute_imitation_reward(
    ...
    diff_joints = torch.stack([diff_level_1_pos_dist, diff_level_2_pos_dist]).mean(dim=0) * 100.
    diff_ft = torch.stack([diff_thumb_tip_pos_dist, diff_index_tip_pos_dist, diff_middle_tip_pos_dist,
                         diff_pinky_tip_pos_dist, diff_ring_tip_pos_dist]).mean(dim=0) * 100.

    diff_obj_pos = diff_obj_pos_dist * 100.
    diff_obj_rot = diff_obj_rot_angle.abs() / np.pi * 180
    diff_joints = diff_joints
    diff_ft = diff_ft
    ...
    return reward_execute, reset_buf, succeeded, failed_execute, reward_dict, error_buf, diff_obj_pos, diff_obj_rot, diff_joints, diff_ft
